...
void setup() {
  ...
  #ifdef useStatusLED //FastLED
    FastLED.addLeds<LED_TYP, LED_PIN, GRB>(leds, ANZAHL_LEDS).setCorrection(TypicalLEDStrip);
    // FastLED.addLeds<LED_TYP, LED_PIN, RGB>(leds, ANZAHL_LEDS);
  ...
  Serial.println(F("TonUINO Version 2.1.1"));
  Serial.println(F("created by Thorsten Voß and licensed under GNU/GPL."));
  Serial.println(F("Information and contribution at https://tonuino.de.\n"));
  Serial.println(F("changed by Marcus Schwager"));
  // Busy Pin
  ...
  #ifdef useStatusLED
    Serial.println(F("LEDshow läuft."));
    LEDshow();
  #endif
...
void loop() {
  do {
    ...
    if ((pauseButton.pressedFor(LONG_PRESS) || upButton.pressedFor(LONG_PRESS) || downButton.pressedFor(LONG_PRESS)) && pauseButton.isPressed() && upButton.isPressed() && downButton.isPressed()) {
      ...
      do {
        readButtons();
        switchOnLeds(ANZAHL_LEDS, INIT_COLOR);
    ...
    //STATUSLED
    if (isPlaying()) {
      switchOnLeds(ANZAHL_LEDS, PLAY_COLOR);
    } else {
      switchOnLeds(ANZAHL_LEDS, WAIT_COLOR);
    }
...
void writeCard(nfcTagObject nfcTag) {
  ...
  byte buffer[16] = {0x13, 0x37, 0xb3, 0x47, // 0x1337 0xb347 magic cookie to
                    ...
                    */
    byte buffer[16] = {0x15, 0x99, 0x67, 0x74, // 0x1337 0xb347 magic cookie to
                     // identify our nfc tags
                     0x02,                   // version 1
                     nfcTag.nfcFolderSettings.folder,          // the folder picked by the user
                     nfcTag.nfcFolderSettings.mode,    // the playback mode picked by the user
  ...
  // byte size = sizeof(buffer); //not used
